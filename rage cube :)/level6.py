...
pygame.display.set_caption("Rage Runner - Level 6")
...
flip_timer = 0
flip_interval = 4000  # flip every 4 seconds

def game_loop(level):
    global player_x, player_y, is_jumping, jump_count, obstacles, control_reversed, flip_timer
    ...
    while run_game:
        ...
        # Reverse controls every few seconds
        now = pygame.time.get_ticks()
        if now - flip_timer > flip_interval:
            control_reversed = not control_reversed
            flip_timer = now
        ...
        # Spawn more evil obstacles
        if len(obstacles) < 14 and random.random() < 0.3:
            obs_x = random.randint(0, screen_width - 40)
            dx = random.choice([-4, -3, -2, 2, 3, 4])
            is_fake = random.random() < 0.15  # 15% are fake
            obstacles.append({
